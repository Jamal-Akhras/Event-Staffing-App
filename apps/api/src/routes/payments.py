...
class PaymentQuoteResponse(BaseModel):
    ...
    estimated_processing_fee: Decimal
...
def quote_payment(
    ...
    if request.service_charge_rate is not None:
        service_rate = request.service_charge_rate
    fee_policy = request.processing_fee_policy or get_env("PROCESSING_FEE_POLICY", "pass_through")

    labor_total = money(request.labor_total)
    service_charge = money(labor_total * service_rate)
    ...
    return PaymentQuoteResponse(
        ...
        estimated_processing_fee=processing_fee,
        platform_net_revenue=money(platform_net),
...
def _processing_fee(subtotal: Decimal, method: str, fee_policy: str, rate_card: dict) -> Decimal:
    ...
    if fee_policy == "pass_through":
        ...
        return money(gross_total - subtotal)
    return money((subtotal * percent) + fixed)
...
def _capped_fee(
    ...
    fee = money((amount * percent) + fixed)
