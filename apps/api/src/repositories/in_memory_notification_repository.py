...
class InMemoryNotificationRepository:
    ...
    def list_for_recipient(
        ...
        if recipient_kind == "venue" and viewer_user_id is not None:
            page = [
                replace(n, read=(n.notification_id, viewer_user_id) in self._receipts)
                for n in page
            ]
        return page
    ...
    def unread_count(
        ...
        return sum(
            not item.read
            for item in self.list_for_recipient(
                recipient_kind, recipient_id, 10000, viewer_user_id=viewer_user_id
            )
        )
    ...
    def mark_read(
        ...
        if notification is None or _recipient_id(notification, recipient_kind) != recipient_id:
            return False
        if recipient_kind == "venue" and viewer_user_id is not None:
            self._receipts.add((notification_id, viewer_user_id))
            return True
    ...
    def mark_all_read_for_recipient(
        ...
        if recipient_kind == "venue" and viewer_user_id is not None:
            ...
            for notification_id, notification in self._notifications.items():
                if (
                    _recipient_id(notification, recipient_kind) == recipient_id
                    and (notification_id, viewer_user_id) not in self._receipts
                ):
                    self._receipts.add((notification_id, viewer_user_id))
                    count += 1
            return count
